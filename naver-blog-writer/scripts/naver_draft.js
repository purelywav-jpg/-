...
function firstTextOf(blocks) {
  const t = blocks.find((b) => b.type === 'text');
  return t ? t.text : '';
}

function makeResult(draft) {
  return {
...
async function readTitleText(frame) {
  return frame.locator('.se-title-text').first().innerText();
}

async function readBodyParagraphs(frame) {
...
async function fixTitle(frame, title) {
  await writeTitle(frame, title);
}

async function prependMissingFirstLine(frame, firstLine) {
  const page = frame.page();
...
async function insertTextBlock(frame, text) {
  const page = frame.page();
...
async function insertSubtitleBlock(frame, text, result) {
  const page = frame.page();
...
async function insertImageBlock(frame, block) {
  const page = frame.page();
...
async function writeBlock(frame, block, nextBlock, result) {
  const page = frame.page();
  switch (block.type) {
    case 'text':
...
async function insertVideo(frame, draft, result) {
  const page = frame.page();
...
async function insertPlace(frame, place, result) {
  const page = frame.page();
  ...
  try {
    ...
    const confirmBtn = frame.locator('button[class*="confirm"]:not([disabled])').first();
    ...
    const closeBtn = frame
      .locator('[class*="se-popup-button-close"], [class*="place_layer"] [class*="close"]')
      .first();
...
async function openPublishPanelAndSetTags(frame, tags, tagLimit, result) {
...
async function closePanelKeepClosed(page) {
...
function printResultBlock(result, dryRun) {
  const line = (label, r) => {
